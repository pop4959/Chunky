...
public final class Chunky extends JavaPlugin {
    ...
    private void printUpdate(World chunkWorld, int chunkX, int chunkZ) {
        ...
        if (totalChunks.get() == finishedChunks.get()) {
            ...
            message = String.format(printUpdateFormat, world, chunkNum, percentDone, etaHours, etaMinutes, etaSeconds, speed, chunkX, chunkZ);
        }
            ...
            while (!chunkLocations.isEmpty()) {
                if (working.get() < FREQ) {
                    ...
                    if (PaperLib.isChunkGenerated(world, chunkLocation.x, chunkLocation.z)) {
                        printUpdate(world, chunkLocation.x, chunkLocation.z);
                        continue;
                    }
                    working.getAndIncrement();
                    PaperLib.getChunkAtAsync(world, chunkLocation.x, chunkLocation.z).thenAccept(chunk -> {
